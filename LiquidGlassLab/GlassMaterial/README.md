...
Each sample carries the restampable transplant groups: every replay-owned typed shader input including captured nils, the render-bounds group, both untinted color grades with their scalar inputs, and the full rim payload — a flat context resolves zero-alpha rim colors, so the gate alone would open onto an invisible highlight. Display/runtime-owned inputs are deliberately omitted: for example, `inputMaxHeadroom` is an animated `9999` sentinel on a virtual display but a constant finite value such as `1.2` on an HDR display, so the Catalog and strength controller leave the live system value untouched. The atlas is `Codable`: capture once, certify one snapshot per macOS major, and retain exact-build/display metadata only for diagnostics. The chosen product policy values a stable major-version look over following minor Recipe drift; a handful of resolved fields are display-sensitive, but a display change alone does not invalidate the catalog.

One transplant group cannot be restamped and falls on the host layout: **window room**. The backing surface hard-clips everything at the window frame, so a glass that fills its panel edge-to-edge clips bounds-extending output even when the backdrop layer retains its native internal sampling margin. With `hasOuterShadow = true`, `requiredWindowInset` reports the native room needed by the complete outer rendering. With `hasOuterShadow = false`, the product deliberately keeps native backdrop sampling but requests a uniform 1pt safety inset, containing bounds-extending output and preventing edge-sample flashing without collapsing background sampling to the exact glass edge. Respond to `onRequiredWindowInsetChange` and keep the visual content size fixed when growing the panel; deriving the glass bounds back from the expanded panel can create a size/inset feedback loop. Window dragging, snapping, and hit testing should continue to use the visual glass frame rather than the larger transparent panel frame.
...
Targeted author visual acceptance of the compact product HUD path is complete on both macOS 26 and macOS 27, including size-dependent rendering. Controlled Dock dragging additionally established a uniform 1pt minimum safety inset with Outer Shadow disabled; an edge-to-edge 0pt room flashes on macOS 26.
