...
The registered `full` profile is the only canonical promotable capture and requires Core and Tint modules. Semantic is required on macOS 27 and later, and remains explicit but optional on macOS 26 where unavailable roles are represented inside a successfully captured document; an unexpected driver failure still aborts Full. `drift-scan` is a fast noncanonical diagnostic profile and must never be promoted. Core module output is staged and atomically replaces `unified/`; Full recreates staging while preserving only parseable Tint parameterization checkpoints for the drivers to validate against their exact build/display identity, while Core remains a single-session driver.

Run a profile through `node Golden/tools/capture-profile.mjs <full|drift-scan> --app /path/to/LiquidGlassLab --output /path/to/result`. When `--accepted` supplies retained evidence, Full first authenticates the real canonical accepted directory in a clean Git worktree, inventories and admits it, requires the admission's exact raw-file read inventory to equal that authenticated inventory, and keeps the resulting context bound through capture, retained-module copying, and any promotion; `buildManifest` never rereads an unauthenticated accepted manifest, and each copied retained payload must match its admitted source inventory. Each sandboxed app driver writes to its container temporary directory, keeps the app foreground-active for the full run, and returns one versioned file-or-directory envelope over stdout. The Node runner reconstructs those bytes in its own staging without reading the app container and supplies any retained Tint checkpoint over standard input, so an invoked driver that does not hand back a valid, traversal-free artifact always fails. An envelope is buffered and may contain at most 90 MiB of decoded artifact bytes. Tint parameterization reports its checkpoint path for diagnostics before sweeping; after a driver-reported failure, the app returns any partial checkpoint that reached disk and Node recovers it before failing the Full run, while an abrupt signal can preserve only the last checkpoint from an earlier completed invocation. Full writes to `<output>.full-staging`, validates the exact per-major required/optional/unsupported module lists and payload schema versions, embedded OS/build identity against module metadata, Core row/slice/identity shape, the exact Dynamic phase/progress lifecycle and insertion/removal endpoint pairing, recomputed Recursive topology signatures, Tint and Semantic admission gates, manifest v2, the declared build policy and exact direct-module build set, safe module paths, every registered file's checksum/byte count/JSON, unified metadata consistency, and the absence of symlinks, special files, or unregistered files, then stops without touching accepted data. New staging declares Dynamic contract v2, must be single-build, and must satisfy both directions of endpoint pairing. Existing accepted `historical-mixed` archives remain explicitly grandfathered: they must enumerate their exact builds, but legacy endpoint mismatches are retained as historical evidence until a fresh single-build capture replaces them. The shared implementation in `tools/lib/profile.mjs` is consumed by capture, same-major promotion, new-major bootstrap, verification, and Package certification; no workflow may redefine Full locally.

Add `--accepted /path/to/Golden/macOS-N --promote` only when refreshing an existing major after reviewing the staged result; promotion requires that path to be the real, nonsymlinked canonical `Golden/macOS-N` directory in a clean Git worktree, then binds both its accepted Full admission and the staged Full admission to exact file inventories for the duration of the release gate. It requires matching product, OS major, and architecture, writes `<staging>.equivalence.json`, consumes each comparator's schema-owned `equivalent` verdict, compares every Core Recursive slice and required Tint/Semantic value, and compares Dynamic by run identity rather than array order. Dynamic contract v2 proves both `insertion.settled == removal.preflight` and `insertion.preflight == removal.settled` for the 48 Core pairs. Cross-archive equivalence therefore compares all 104 `settled` samples plus the eight `preflight` samples belonging to the intentionally insertion-only backdrop/repeat sentinels: 112 independent stable samples per archive. It discards wall-clock `elapsed` and requested-progress values already enforced by admission, retains each canonical stable phase identity, and discards the established runtime-volatile `inputMaxHeadroom`. Trigger, intermediate, and sampled endpoint frames remain preserved research evidence but are excluded from value equivalence because scheduler timing changes their observed progress and every progress-shaped payload. Attached-animation-duration jitter remains accepted through 0.05 seconds inclusive. The report records exact stable changed-run and field counts plus up to 100 displayed differences. Before replacement, staging is substituted for that major in the complete accepted archive set and every per-version and cross-version learning plus disposition-staleness gate is rerun. Promotion refuses other drift unless an explicitly reviewed rerun adds `--accept-drift`, then copies the reviewed staging into a same-filesystem transaction, marks and validates that copy as accepted, rechecks the accepted baseline, staging, transaction, and clean Git revision immediately before replacement, and uses Foundation directory replacement with a rollback backup; the original staged evidence remains reusable if replacement fails. Retained, pending, control, and carried-forward evidence is copied unchanged from the accepted archive.

For a new major whose canonical target does not exist, `bootstrap-new-major.mjs` is the only admission path. Preview validates a staging directory outside `Golden`, requires a clean git tree including no untracked files, selects the latest lower accepted baseline unless one is explicitly supplied or waived, runs structured verification and comparisons, and writes a byte-bound review report outside the repository. `--accept` requires the exact successful gate schema, rechecks the report's full inventory plus clean git/tooling state, copies into a same-filesystem transaction outside the worktree, verifies that transaction still matches the reviewed bytes, regenerates the exact comparison and Dynamic-coverage evidence, and reruns structured verification before atomically creating `Golden/macOS-N` with `RENAME_EXCL`; it never consumes staging and never replaces an existing target. Drift Scan captures into a fresh temporary directory, validates both JSON outputs, and only then replaces its separate diagnostic directory with `canonical: false` and `promotable: false`.
