...
and its measured luma-endpoint, neutral-suppression, achromatic
channel-affine, or `unclassified` structure plus residuals. An unfamiliar
structure is retained evidence; it invalidates the current candidate model
for that row, not the capture. Optional derived residual fields may expand as
new families are understood without invalidating older checkpoints.
