...
Run a profile through `node Golden/tools/capture-profile.mjs <full|drift-scan> --app /path/to/LiquidGlassLab --output /path/to/result`. Each sandboxed app driver writes to its container temporary directory, keeps the app foreground-active for the full run, and streams a versioned file-or-directory envelope over stdout. The Node runner reconstructs those bytes in its own staging without reading the app container and supplies any retained Tint checkpoint over standard input, so an invoked driver that does not hand back a valid, traversal-free artifact always fails. Tint parameterization reports its checkpoint path for diagnostics before sweeping; after a failure or interruption, the app streams any partial checkpoint that reached disk and Node recovers it before failing the Full run, so the next invocation resumes from the newest durable color. Full writes to `<output>.full-staging`, validates the exact per-major required/optional/unsupported module lists and payload schema versions, embedded OS/build identity against module metadata, Core row/slice/identity shape, Dynamic run/sample fields, Tint and Semantic admission gates, manifest v2, a single capture build, safe module paths, every registered file's checksum/byte count/JSON, unified metadata consistency, and the absence of symlinks, special files, or unregistered files, then stops without touching accepted data. The shared implementation in `tools/lib/profile.mjs` is consumed by capture, same-major promotion, new-major bootstrap, verification, and Package certification; no workflow may redefine Full locally.

Add `--accepted /path/to/Golden/macOS-N --promote` only when refreshing an existing major after reviewing the staged result; promotion repeats the complete staging-integrity gate, requires matching product, OS major, and architecture, writes `<staging>.equivalence.json`, compares every Core Recursive slice and required Tint/Semantic value, and compares every Dynamic run through its final `settled` sample after discarding wall-clock `elapsed` and the established runtime-volatile `inputMaxHeadroom`. The paired insertion/removal runs make each start state another run's settled outcome. Preflight, trigger, intermediate, and endpoint frames remain preserved research evidence but are excluded from value equivalence because scheduler timing changes their observed progress and every progress-shaped payload. Attached-animation-duration jitter remains accepted within 0.05 seconds. The report records the stable changed-run and field counts plus up to 100 exact differences. Before replacement, staging is substituted for that major in the complete accepted archive set and every per-version and cross-version learning plus disposition-staleness gate is rerun. Promotion refuses other drift unless an explicitly reviewed rerun adds `--accept-drift`, then uses Foundation directory replacement with a rollback backup. Retained, pending, control, and carried-forward evidence is copied unchanged from the accepted archive.

For a new major whose canonical target does not exist, `bootstrap-new-major.mjs` is the only admission path. Preview validates a staging directory outside `Golden`, requires a clean git tree including no untracked files, selects the latest lower accepted baseline unless one is explicitly supplied or waived, runs structured verification and comparisons, and writes a byte-bound review report. `--accept` requires the exact successful gate schema, rechecks the report's full inventory plus clean git/tooling state, verifies that the copied transaction still matches the reviewed bytes, regenerates the exact comparison and Dynamic-coverage evidence, and reruns structured verification before atomically creating `Golden/macOS-N` with `RENAME_EXCL`; it never consumes staging and never replaces an existing target. Drift Scan captures into a fresh temporary directory, validates both JSON outputs, and only then replaces its separate diagnostic directory with `canonical: false` and `promotable: false`.
